...
//
// RecoveryCoordinator implementasyonu. Windows'a özel (encode_subsystem gibi
// yalnızca WIN32 altında derlenir). Davranış, Pipeline::Impl'in eski
// handle_device_lost() koduyla BİREBİR aynıdır — sıra korunmuştur (Aşama 9 saf
// çıkarma; ISource wiring'de CaptureSubsystem& imzası ExistingDesktopSource&'a
// geçti, reinit Config'i artık adapter'da saklı). dbglog / seh_connection_lost,
// pipeline.cpp'deki anon-namespace eşdeğerleriyle aynı (modül bağımsızlığı).
...
// ISource wiring — silinen CaptureSubsystem sürümüyle BİREBİR akış (sıra
// korunmuştur). Farklar yalnızca mekanik: cihaz seçimi metadata().device
